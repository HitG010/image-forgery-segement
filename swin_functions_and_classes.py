...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # Initialize a list to store feature maps for each stage
        features = []
        
        # Process through each layer of the Swin Transformer
        for layer in self.layers:
            x = layer(x)
            features.append(x)  # Store the feature map for FPN

        # # Normalize and pool the final output
        # x = self.norm(x)  # (B, L, C)
        # x = self.avgpool(x.transpose(1, 2))  # (B, C, 1)
        # x = torch.flatten(x, 1)
        
        # return x, features  # Return both the final output and the feature maps for FPN
        return features  # Return only the feature maps for FPN
...
# print(features[0].shape) # (1, 14, 14, 384)